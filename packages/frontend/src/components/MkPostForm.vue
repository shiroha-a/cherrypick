...
async function post(ev?: MouseEvent) {
	...
	if (textareaEl.value) {
		const height = store.s.useClassicPostForm === true ? '90px' : '140px';
...
watch(() => store.s.useClassicPostForm, (newVal) => {
}, { immediate: true });
...
.root {
  ...
  max-width: 800px;
	...
	&.modal {
		width: 100%;
		max-width: v-bind('store.s.useClassicPostForm ? "520px" : "640px"');
		overflow-x: clip;
		overflow-y: auto;
	}
}
...
.text {
  ...
  min-width: 100%;
  width: 100%;
  min-height: v-bind('store.s.useClassicPostForm ? "90px" : "140px"');
  height: 100%;
}

@container (max-width: 500px) {
  .text {
    min-height: v-bind('store.s.useClassicPostForm ? "80px" : "120px"');
  }
}
...
.footerLeft {
  ...
  @if (v-bind('store.s.useClassicPostForm')) {
    width: auto;
    overflow-x: auto;
    overflow-y: hidden;
  } @else {
